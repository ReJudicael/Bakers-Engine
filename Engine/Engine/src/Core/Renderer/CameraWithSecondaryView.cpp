...
{
	rttr::registration::class_<CameraWithSecondaryView>("Camera with secondary view")
	.constructor()
	.property("Target fbo", &CameraWithSecondaryView::m_targetFbo)
	.property("fbo type", &CameraWithSecondaryView::m_typeFbo)
	.property("Secondary view pos", &CameraWithSecondaryView::GetViewPos, &CameraWithSecondaryView::SetViewPos)
	(
		rttr::metadata(MetaData_Type::SHOW_IN_EDITOR, "ShouldDisplayPosDataInInspector")
	)
	.property("Secondary view size", &CameraWithSecondaryView::GetViewSize, &CameraWithSecondaryView::SetViewSize)
	(
		rttr::metadata(MetaData_Type::SHOW_IN_EDITOR, "ShouldDisplayPosDataInInspector")
	)
	.method("ShouldDisplayPosDataInInspector", &CameraWithSecondaryView::ShouldDisplayPosDataInInspector);
}
...
void CameraWithSecondaryView::Draw(const std::list<Core::Datastructure::IRenderable*>& renderables)
{
	...
	if (fbo->Size[2] != static_cast<int>(m_viewSize.x * m_cameraWidth) || fbo->Size[3] != static_cast<int>(m_viewSize.y * m_cameraHeight))
	{
		...
		static_cast<ICamera*>(fbo->userPtr)->Resize(m_viewSize.x * m_cameraWidth, m_viewSize.y * m_cameraHeight);
	}
}
